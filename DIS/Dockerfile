...
#
RUN apt-get update
RUN apt install -y make libssl-dev libghc-zlib-dev libcurl4-gnutls-dev libexpat1-dev gettext unzip nano vim
RUN cd /tmp &&  \
...
WORKDIR /KAIST_SODAS/DIS/
RUN git checkout September_Test
RUN npm install
WORKDIR /KAIST_SODAS/DIS/Daemon
ENV DEBUG=sodas:*
